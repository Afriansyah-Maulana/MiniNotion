...
export default function DashboardView({ tasks, allTasks, courses }) {
  ...
  const weekdayEn = new Date().toLocaleDateString("en-US", { weekday: "long" });
  const weekdayId = new Date().toLocaleDateString("id-ID", { weekday: "long" });
  const displayWeekday = new Date().toLocaleDateString(
    i18n.language === "id" ? "id-ID" : "en-US",
    { weekday: "long" },
  );
  const todayCourses = courses.filter((course) =>
    course.schedule.toLowerCase().includes(weekdayEn.toLowerCase()) ||
    course.schedule.toLowerCase().includes(weekdayId.toLowerCase())
  );
  const now = new Date();
